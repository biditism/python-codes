...
tail_freedom=0.2 #Percentage freedeom for tail to vary later
...
dq_params.add('first_A',0.5,vary=True,min=0.01,max=0.8)
...
Itot_model= Itot_model + lm.Model(my.distribution_T2_decay,prefix='tail_')


dq_params.add('tail_T2',tail_result['T2'],vary=True,
              min=tail_result['T2']*(1-tail_freedom),
              max=tail_result['T2']*(1+tail_freedom))
dq_params.add('tail_sigma',0.5,vary=True,min=0.01,max=1.0)
...
dq_params.add('second_T2',21,vary=True,min=10,max=40)
...
tot_fitted= my.fit_tot_only(df,dq_params,Itot_model,connectivity,
                            method='leastsq')
...
sim_fit=lm.Minimizer(
    ...
    fcn_args=(df,Idq_model,Itot_model,connectivity,
              T2_penalty,Dres_penalty,dq_cutoff),
    max_nfev=10000000)
...
plt.plot(df["Time"],df['I_DQ']-fitted_points_dq['Total_Fit_'],
         label='DQ Residual')

plt.plot(df["Time"],df['I_tot']-fitted_points_tot['Total_Fit_'],
