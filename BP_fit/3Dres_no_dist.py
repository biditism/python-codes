...
if __name__ == '__main__':
    ...
    chi_sqr_map=False
    map_parameters= ['first_A','second_A','third_A','first_Dres','second_Dres']
    ...
    if chi_sqr_map is True:
        ...
        for parameter in map_parameters:
            ...
            lower = max(min(sim_fitted.params[parameter] - 5*sim_fitted.params[parameter].stderr,sim_fitted.params[parameter].value * 0.5),sim_fitted.params[parameter].min)
            higher = min(max(sim_fitted.params[parameter] + 5*sim_fitted.params[parameter].stderr,sim_fitted.params[parameter].value * 1.5),sim_fitted.params[parameter].max)
            
            if parameter in ['first_A','second_A','third_A']:
                lower = 0.1
                higher = 0.9
